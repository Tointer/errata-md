...
import { listStories, createFragment, createStory, updateFragmentVersioned } from '@/server/fragments/storage'
...
describe('md-files loading and parsing', () => {
  ...
  it('preserves a visible fragment id when its filename changes', async () => {
    const tmp = await createTempDir()

    try {
      const story = makeStory('story-visible-rename')
      await createStory(tmp.path, story)
      await createFragment(tmp.path, story.id, makeFragment('ch-io-dren', {
        type: 'character',
        name: 'Io Dren',
      }))

      await updateFragmentVersioned(tmp.path, story.id, 'ch-io-dren', { name: 'Io Vale' })

      const raw = await readFile(
        join(getMarkdownStoryRoot(tmp.path, story.id), 'Characters', 'Io Vale.md'),
        'utf-8',
      )
      expect(raw).toContain('id: "ch-io-dren"')
      const loaded = await loadMarkdownFragmentById(tmp.path, story.id, 'ch-io-dren')
      expect(loaded?.name).toBe('Io Vale')
